...
// recherche dans les ingrédients, arr= globalrecherche, input = le tag retenu
// retourne un tabelau des recettes qui matchent
rechercheIngredient = (arr, input) => {
  console.log('appel rechercheIngredient : input : ' + input)
  const recipesIngredients = arr.map((element) => {
    const { ingredients } = element;
    const allIngredient = ingredients.map((el) => el.ingredient);
    return allIngredient.filter((item) => {
      return remplaceCarSpeciaux(item).includes(input);
    });
  });

  const tabIndexQuiMatchentTmp = [];
  //const EstRempli = (element) => element.length > 0;
  for (const item of recipesIngredients) {
    if (item.length> 0) { //if (item.findIndex(EstRempli) === 0) {
      tabIndexQuiMatchentTmp.push(recipesIngredients.indexOf(item));
    }
  }
  const rechercheParIngredient = [];
  for (const i of tabIndexQuiMatchentTmp) {
    rechercheParIngredient.push(arr[i]);
  }

  return [...new Set(rechercheParIngredient)]; //supression doublons
} // rechercheIngredient

// recherche dans les appareils
rechercheAppliance = (arr, input) => {
    const recipesAppliances = arr.filter((element) => {
        return remplaceCarSpeciaux(element.appliance).includes(input);  
    });
  return [...new Set(recipesAppliances)]; //supression doublons
} // rechercheAppliance

// recherche parmis les ustensils
rechercheUstensil = (arr, input) => {
    const recipesUstensils = arr.map((element) => {
    ...
    const tabIndexQuiMatchentTmp = [];
    //const EstRempli = (element) => element.length > 0;
    for (const item of recipesUstensils) {
      if (item.length> 0) { //if (item.findIndex(EstRempli) === 0) {
        tabIndexQuiMatchentTmp.push(recipesUstensils.indexOf(item));
      }
    }
    const rechercheByUstensils = [];
    for (const i of tabIndexQuiMatchentTmp) {
      rechercheByUstensils.push(arr[i]);
    }
   
  return [...new Set(rechercheByUstensils)]; //supression doublons
} //rechercheUstensil